...
section_names = ["xor", "bit conversion", "bit count", "quantization", "hamming distance"]
...
parser.add_argument("--randomization", "-rnd", choices=["mutate", "full", "sparse"], default=randomization,
                    help="Specify which randomization method was used to generate the data. Used to locate the correct datafile. Default is 'mutate'.")
...
filename_pattern = os.path.join(datapath, f"bits_{n_vector}x{n_elements}_{{}}_{randomization}.hdf5")
...
for (implementation, impl_info) in implementations.items():
    if impl_info["skip"]:
        print(f"Skipping '{implementation}'.")
        continue
